...
const ProfessionalPlay = () => {
  ...
  return (
    <div className="professional-play">
      <div className="container">
        {/* Header */}
        <motion.div
          className="professional-play-header"
          initial={{ opacity: 0, y: -30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
        >
          <motion.h1 
            className="professional-page-title"
          ...
          <motion.div
            className="professional-game-controls"
          ...
          >
            ...
            <motion.select
              ...
              className="professional-difficulty-select"
        ...
        {/* Main Game Layout */}
        <div className="professional-game-layout">
          {/* Left Panel - Game Info */}
          <motion.div
            className="professional-left-panel"
            initial={{ opacity: 0, x: -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.6, duration: 0.6 }}
          >
            <div className="professional-game-info">
              <div className="professional-info-header">
                <h3>Game Info</h3>
                <div className={`professional-status-indicator ${gameStatus.toLowerCase()}`}>
                  <span className="professional-status-dot"></span>
                  {gameStatus}
                </div>
              </div>
              
              <div className="professional-info-stats">
                <div className="professional-info-item">
                  <span className="professional-info-label">Moves:</span>
                  <span className="professional-info-value">{moveHistory.length}</span>
                </div>
                
                <div className="professional-info-item">
                  <span className="professional-info-label">Time:</span>
                ...
                <div className="professional-info-item">
                  <span className="professional-info-label">AI Level:</span>
                  <span className="professional-info-value">{aiDifficulty}</span>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            className="professional-center-panel"
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ delay: 0.8, duration: 0.8 }}
          >
            <div className="professional-board-container">
              <motion.div
                className="professional-board-wrapper"
                initial={{ rotate: 0 }}
            ...
            <motion.div
              className="professional-board-controls"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <motion.button
                onClick={copyFEN}
                className="professional-btn professional-btn-secondary professional-btn-small"
          ...
          <motion.div
            className="professional-right-panel"
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 1, duration: 0.6 }}
          >
            <div className="professional-move-history">
              <div className="professional-history-header">
                <h3>📜 Move History</h3>
                <span className="professional-move-count">{moveHistory.length} moves</span>
              </div>
              
              <div className="professional-moves-list">
                <AnimatePresence>
                  {moveHistory.slice(-8).map((move, index) => (
                    <motion.div
                      key={moveHistory.length - 8 + index}
                      className={`professional-move-item ${index % 2 === 0 ? 'white-move' : 'black-move'}`}
                    ...
                    >
                      <span className="professional-move-number">
                        {Math.floor((moveHistory.length - 8 + index) / 2) + 1}.
                      </span>
                      <span className="professional-move-notation">{move}</span>
                    </motion.div>
                  ))}
                </AnimatePresence>
              </div>
...
export default ProfessionalPlay
